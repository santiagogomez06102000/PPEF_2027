...
export function Cubo() {
  ...
  const CUBE_CONFIG = {
    faces: [
      {
        ...
        full_desc: `<p>Es el documento elaborado por la SHCP en el que se presentan los aspectos más relevantes del marco macroeconómico y los objetivos de finanzas públicas para el cierre del año en curso y el siguiente ejercicio fiscal.</p>
    ...
    <p>Con este documento se inicia el diálogo con el Congreso de la Unión respecto del proceso presupuestario para el siguiente ejercicio fiscal, dando inicio a su correspondiente ciclo presupuestario. Se entrega, a más tardar el 1° de abril de cada año, al Congreso de la Unión y se publica en la página web de la SHCP para su consulta pública.</p>`,
      ...
      },
      {
        ...
        color: 0xfdd353,
        opacity: 0.8,
        label: '¿Qué pasó el ciclo anterior?',
        short_desc: 'Panorama general',
        full_desc:
  ...
  class SceneManager {
    constructor(containerId) {
      ...
      // 1. Establecer pixel ratio nativo del monitor (limitado a 2 para no saturar GPU)
      const dpr = Math.min(window.devicePixelRatio || 1, 2)
      this.renderer.setPixelRatio(dpr)

      // 2. Primer ajuste de tamaño: false evita que Three.js inyecte estilos inline
      ...
      // 3. Forzar que el canvas ocupe siempre el 100% del contenedor vía CSS
      ...
      // 4. ResizeObserver detecta cambios reales del contenedor (incluyendo scroll-mode)
    ...
    setupResizeObserver() {
      ...
      // Fallback: si el usuario hace zoom (Ctrl +/-), el DPR cambia
    ...
    onResize(width, height) {
      ...
      // false = no modificar canvas.style.width/height, respeta el CSS 100%
  ...
  class CubeController {
    ...
    init() {
      ...
      // Grosor del borde (radio del cilindro):
      ...
      for (let i = 0; i < positions.length; i += 6) {
        ...
        // Puedes usar CylinderGeometry o BoxGeometry; ambos respetan el grosor en WebGL
        const edgeGeo = new THREE.CylinderGeometry(thickness, thickness, distance, 8)
        const edgeMesh = new THREE.Mesh(edgeGeo, edgeMaterial)

        edgeMesh.position.copy(midPoint)
        edgeMesh.lookAt(v2)
        edgeMesh.rotateX(Math.PI / 2) // Alinea el cilindro con el eje de la arista

        this.cube.add(edgeMesh)
      }
    }
    createMaterials() {
  ...
  class UIManager {
    ...
    updatePanel(face) {
      ...
      /* Agregando fondo de color custom */
      this.verMasBtn.querySelector('.icon').style.backgroundColor =
        `${'#' + face.color.toString(16).padStart(6, '0')}`

      const icon = this.verMasBtn

      this.verMasBtn.onmouseenter = () => {
        icon.style.backgroundColor = '#' + face.color.toString(16).padStart(6, '0')
      }

      this.verMasBtn.onmouseleave = () => {
