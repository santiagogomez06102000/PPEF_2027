...
<template>
  <li class="w-full h-full lista-nav">
    <div v-if="item?.submenu?.length > 0" class="flex h-full w-full items-center">
      ...
      <div
        v-if="props.width >= 1024"
        @mouseenter="handleMostrarSubmenu(true)"
        @mouseleave="handleMostrarSubmenu(false)"
        class="contenedor-menu relative"
        :class="props.width >= 1024 ? 'sublista-nav' : ''"
      >
        <button
          @click="handleMostrarSubmenu(!submenu)"
          type="button"
          class="btn-menu leading-none w-full h-full text-lg font-bold transition-all duration-200 hover:px-4 transition-colors"
          :class="
            item?.nivel === 1
        ...
        >
          ...
          <div
            v-show="submenu"
            class="flex w-auto flex-col opacity-100 transition-all contenedor-submenu"
          >
      ...
      <div v-else class="w-full">
        ...
        >
          ...
          <svg
            ...
            :class="{ 'rotate-180': openDropdown === item.id }"
        ...
        <div
          class="mt-0 space-y-1 pl-4 acordion"
          :class="props.openDropdown === item.id ? 'mostrar-acordion' : ''"
        >
    ...
    <a
      v-else
      :href="item?.url"
      class="contenedor-menu flex leading-none h-full items-center px-3 text-start text-lg font-bold transition-all duration-200 hover:px-4 transition-colors"
      :class="
        item?.nivel === 1
          ? `text-[var(--bg-nav)] hover:text-white  hover:bg-[var(--bg-nav)]`
