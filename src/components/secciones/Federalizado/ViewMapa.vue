...
async function obtenerDatos() {
  ...
  const entidadesFed =
    await fetchPublicJson<EntidadFederativa[]>(
...
<template>
  <section class="w-full">
    <div class="w-full flex flex-col-reverse lg:flex-row gap-8">

      <!-- MAPA -->
      <div class="flex-1 w-full rounded-xl shadow-xl mapa-container">
        <MapaMexico />
      </div>

      <!-- MASCOTA -->
      <div class="w-full lg:w-[30%] flex flex-col items-center justify-center">
        <img :src="baseUrl + 'mascota/7_impulso.png'" alt="" />

        <div>
          <h3 class="text-[var(--color-blanco)]">
            Selecciona un estado para conocer el monto designado
          </h3>
        </div>
      </div>
